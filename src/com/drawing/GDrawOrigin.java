...
public class GDrawOrigin implements GShape {
	...
	public GDrawOrigin(float w, float h )
	{
		...
		float vertex2f[] = new float[] { -this.width / 2, 0f, // 0: left top point
				this.width / 2, 0f, // 2: bottom right point
		...
		vertex2f = new float[] { 0f, this.height / 2, // 0: left top point
				0f, -this.height / 2, // 2: bottom right point
				0.2f, 0.9f, .6f, // 4: main color
				.1f, 0.5f, .90f }; // 7: outline color
